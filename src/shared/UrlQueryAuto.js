...
const UrlQueryAuto = ({autoOptions = [], label = '', placeholder = ''}) => {
  ...
  const searchQuery = Array.isArray(originalQuery.search) ? originalQuery.search : originalQuery.search ? [originalQuery.search] : [];

  const pushNewQuery = (autoValues) => {
    const newQuery = Object.assign(originalQuery, {search: autoValues});
    const newQueryStringify = '?' + stringify(newQuery, {arrayFormat: 'comma'});
